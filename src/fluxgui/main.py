...
# Handle different appindicator implementations
try:
    gi.require_version('AyatanaAppIndicator3', '0.1')
    from gi.repository import AyatanaAppIndicator3 as appindicator
except:
    print('Falling back to regular appindicator...')
    try:
        gi.require_version('AppIndicator3', '0.1')
        from gi.repository import AppIndicator3 as appindicator
    except:
        print('Failed to import any appindicator, exiting...')
        sys.exit(1)

class FluxGUI:
    """
    FluxGUI initializes/destroys the app and manages the main window
    and tray icon.
    """
    def __init__(self):
        try:
            ...
            self.controllers = {
                "xflux": XfluxSettings(self.settings),
...
class Indicator:
    """
    System tray icon and related menu for controlling FluxGUI.
    """
    ...
    def create_menu(self):
        menu = gtk.Menu()
        self.add_menu_item("Pause f.lux", self._toggle_pause, menu, MenuItem=gtk.CheckMenuItem)
...
class Preferences:
    """
    Preferences window for configuring FluxGUI.
    """
    def __init__(self, settings, controllers):
        ...
        self.window = self.connect_widget("window1", self.delete_event, connect_event="delete-event")
        self.notebook = self.connect_widget("notebook", self.switch_page, connect_event="switch-page")
        self.xflux_tab = XfluxTab(self.window, controllers, self.settings, self.connect_widget)
        self.redshift_tab = RedshiftTab(self.window, controllers, self.settings, self.connect_widget)
    ...
    def connect_widget(self, widget_name, connect_target=None, connect_event="activate"):
...
def main():
    try:
        ...
        pass  # Handle unexpected keyboard interrupts
